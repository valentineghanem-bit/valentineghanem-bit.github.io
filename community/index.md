      ...
      <div class="event-grid">
        ...
        {%- assign geo_i = geo_i | plus: 1 -%}
          ...
          <div class="event-media event-media--accordion">
            ...
            <figure class="event-media__item{% if p.contain %} event-media__item--contain{% endif %}"><img src="{{ p.url | relative_url }}" alt="{{ p.caption }}" loading="lazy">{% if p.caption %}<figcaption class="event-media__caption">{{ p.caption }}</figcaption>{% endif %}</figure>
            {% endfor %}
            {% if e.photos.size > 6 %}
